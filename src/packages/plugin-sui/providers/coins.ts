...
const coinTypes = [
    ...
    "0x23e99e646a4071609a2957d0b7c7bac73eaefc2f94640e9d8d54e4ab77f8cd6e::dapy::DAPY",
    ...
    "0xd3ccd5bf9e845177220dadd080aa05bff57706f73afb2f3e3713bcf8fb68b447::mazu::MAZU",
...
export class CoinsProvider {
    ...
    async fetchTokensInfo(): Promise<CoinsInfo> {
        try {
            ...
            const cacheKey = `tokens-${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()}-${date.getHours()}-${roundedMinutes}`;

            const cachedValue =
                await this.getCachedData<CoinsInfo>(cacheKey);
            ...
            const coinsInfo = await this.fetchTokensInfoRetry().catch(
                (error) => {
...
const coinsProvider: Provider = {
    get: async (
        ...
        try {
            const provider = new CoinsProvider(
